...
function createSprites() {
  ...
  score = new Score(ctx, scaleRatio, 1000, itemController);
}
...
function gameLoop(currentTime) {
  ...
  if (!gameover && !player.isPoweredUp && cactiController.collideWith(player)) {
    gameover = true;
    score.setHighScore();
    setupGameReset();
  }
  ...
  if (player.isPoweredUp) {
    cactiController.handleCollisionWithPlayer(player); // 무적 상태일 때 충돌 처리
  }
  ...
  if (collideWithItem && collideWithItem.itemId) {
    ...
    // 특정 아이템(예: ID가 1인 경우)을 먹었을 때 플레이어의 크기 증가 및 무적 상태 발동
    if (collideWithItem.itemId === 1) {
      // 아이템 ID에 따라 조건 설정
      player.powerUp(); // 플레이어 무적 상태 및 크기 증가 발동
    }
  }
